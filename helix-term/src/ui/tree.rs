...
fn render_tree<T: TreeViewItem>(
    ...
    let content = if tree.item.is_parent() {
        let open = tree.is_opened;
        match icons.fs().directory(open) {
            Some(icon) => format!("{icon} {name}/"),
            None => format!("{name}/"),
        }
    } else {
        let icon_opt = icons.fs().from_name(&name);
        let icon_str = icon_opt.or_else(|| {
    ...
    vec![head]
        .into_iter()
        .chain(tree.children.iter().flat_map(|child| {
    ...
    pub fn render(
        ...
        for (i, line) in self.render_lines(area).into_iter().enumerate() {
            ...
            for span in spans {
                ...
                if to_draw > 0 {
                    surface.set_stringn(current_x, y, &span.content, to_draw, span.style);
